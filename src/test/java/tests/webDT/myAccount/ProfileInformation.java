...
/**
 * Created by AbdulazeezM on 7/12/2017.
 */
public class ProfileInformation extends BaseTest {
    WebDriver driver;
    ...
    public void validatePersonalInfoPage() throws Exception {
    ...
    @Parameters(storeXml)
    @Test(priority = 7, groups = {MYACCOUNT, REGRESSION,USONLY,REGISTEREDONLY,PROD_REGRESSION})
    public void validateErrMsg(@Optional("US") String store) throws Exception {
        ...
        AssertFailAndContinue(myAccountPageActions.moreThanEightChar(password,"passwordd","passwordd",errMsgSplChar.get(1)),"Enter the passwors which has more than eight characters and check the error message");
        AssertFailAndContinue(myAccountPageActions.changePasswordUS(password, changePwd.get(0), changePwd.get(0)), "Change the existing password");

    }
    @Parameters(storeXml)
    @Test(priority = 6,groups = {MYACCOUNT, REGRESSION,USONLY,REGISTEREDONLY,PROD_REGRESSION})
    public void editProfileInformation(@Optional("US") String store) throws Exception {
        setAuthorInfo("Azeez");
        ...
        String unSavedError = getDT2TestingCellValueBySheetRowAndColumn("MyAccount","profileInfoUnsavedErr","TermsContent");
      ...
      //DT-33878   AssertFailAndContinue(myAccountPageActions.validatePIError(unSavedError),"Validate the error message while clicking save button");
        /*myAccountPageActions.clickOnPersonalInfoCancelButton();
        myAccountPageActions.clickOnPersonalInfoEditButton();
        */
    ...
    @Parameters(storeXml)
    @Test(priority = 2,groups = {MYACCOUNT, REGRESSION,USONLY,REGISTEREDONLY,PROD_REGRESSION})
    public void validateBirthdaySavingsFields(@Optional("US") String store) throws Exception {
        setAuthorInfo("Azeez");
        ...
        AssertFailAndContinue(myAccountPageActions.faqRedirection(),"Verify the faq redirection related validation");
    }
    @Parameters(storeXml)
    @Test(priority = 3,groups = {MYACCOUNT, REGRESSION,USONLY,REGISTEREDONLY,PROD_REGRESSION})
    public void validateBirthdaySavingsErrMsg(@Optional("US") String store) throws Exception {
        setAuthorInfo("Azeez");
        ...
        AssertFailAndContinue(myAccountPageActions.validateChildMaxCharError(errMsgMaxChar.get(0),errMsgMaxChar.get(1)),"Verify the max character error message is displayed to the user");

    }
    @Parameters(storeXml)
    @Test(priority = 5,groups = {MYACCOUNT, REGRESSION,USONLY,REGISTEREDONLY,PROD_REGRESSION})
    public void addBirthdaySavings(@Optional("US") String store) throws Exception {
        setAuthorInfo("Azeez");
        setRequirementCoverage(store+ " store - Verify if the register user validate all possible error message in the birthday savings fields in my account page");
        ...
        AssertFailAndContinue(myAccountPageDrawerActions.clickViewMyAccount(headerMenuActions, myAccountPageActions), "Clicked on the View My Account link in the My Account Drawer and landed on the My Account page");
        AssertFailAndContinue(myAccountPageActions.clickProfileInfoLink(), "Click on profile information link and check user navigate to appropriate page");
        AssertWarnAndContinue(myAccountPageActions.validateFieldsInProfInfoBeforeAdded(),"Validate the fields present in the profile information page without adding the birthday information");
        AssertFailAndContinue(myAccountPageActions.clickAddBirthdayInfo(), "Click on add birthday button and navigate to appropriate page");
    ...
    @Parameters(storeXml)
    @Test(priority = 4,groups = {MYACCOUNT, REGRESSION,USONLY,REGISTEREDONLY,PROD_REGRESSION})
    public void addAndDeleteChildBirthdaySavings(@Optional("US") String store) throws Exception {
        setAuthorInfo("Azeez");
    ...
    @Parameters(storeXml)
    @Test(groups = {MYACCOUNT, REGISTEREDONLY,USONLY,REGRESSION,PROD_REGRESSION},priority = 0)
    public void addAssociateIDAndPlaceOrder(@Optional("US") String store) throws Exception {
        setAuthorInfo("Azeez");
        ...
        String freeShip = getDT2TestingCellValueBySheetRowAndColumn("Coupons", "FreeShipping", "couponCode");
        ...
        myAccountPageActions.staticWait(4000);
        headerMenuActions.waitUntilElementDisplayed(myAccountPageActions.lnk_AccountOverView,5);
        myAccountPageActions.scrollToTheTopHeader(myAccountPageActions.lnk_AccountOverView);
        myAccountPageDrawerActions.click(myAccountPageActions.lnk_AccountOverView);
        ...
        AssertFailAndContinue(shoppingBagPageActions.giftCardPriceCheck(),"Verify the offer price is not displayed for Giftcard");
    ...
    @Test(dataProvider = dataProviderName, groups = {MYACCOUNT, REGISTEREDONLY})
    public void difStoreMailingAdd(@Optional("US") String store, @Optional("registered") String user) throws Exception {
        setAuthorInfo("Srijith");
        setRequirementCoverage("As a registered user in" + store + " verify that the user is able to save different Mailing Address" +
                "DT-44899");
        Map<String, String> shipDetailUS = getMapOfItemNamesAndQuantityWithCommaDelimited("ShippingDetails", "validBillingDetailsUS");
        Map<String, String> shipDetailCA = getMapOfItemNamesAndQuantityWithCommaDelimited("ShippingDetails", "validShippingDetailsCA");
        ...
        if(store.equalsIgnoreCase("US")){
            ...
            AssertFailAndContinue(myAccountPageActions.addAssociateID("310620",shipDetailCA.get("addressLine1"),shipDetailCA.get("city"), shipDetailCA.get("stateShortName")),"Enter the valid associate ID");
            myAccountPageActions.staticWait(4000);
            headerMenuActions.waitUntilElementDisplayed(myAccountPageActions.lnk_AccountOverView,5);
        }
    }
